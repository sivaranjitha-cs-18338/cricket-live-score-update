...
const { AUTH_HEADER, COOKIE_HEADER, CREDENTIAL_HEADER, DEFAULT_ENV, IS_LOCAL } = constants_1.default;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.credential instanceof credential_1.CatalystCredential ||
                ...
                if ((0, validator_1.isNonEmptyString)(token.access_token)) {
                    ...
                    const tokenOnly = !!req.external && IS_LOCAL === 'true';
                    if (tokenOnly && this.config) {
                        if (!this.tempCredential) {
                            this.tempCredential = new credential_1.JwtTokenCredential({
                                cookie: token.cookie,
                        ...
                        const tokenRes = yield this.tempCredential.getToken();
                        this.setOauthHeader(req.headers, tokenRes.access_token);
                    }
                    else {
...
class CatalystApp extends CatalystAppInternals {
    ...
    jobScheduling() {
        return this.ensureService('job_scheduling', () => {
            const JobSchedulingService = requireModule('./job-scheduling/index')
                .JobScheduling;
            return new JobSchedulingService(this);
        });
    }
    datastore() {
    ...
    pipeline() {
        return this.ensureService('pipeline', () => {
            const pipelineService = requireModule('./pipeline').Pipeline;
            return new pipelineService(this);
        });
    }
    nosql() {
        return this.ensureService('no_sql', () => {
            const noSQLService = requireModule('./no-sql').default;
            return new noSQLService(this);
        });
    }
    stratus() {
        return this.ensureService('stratus', () => {
            const stratusService = requireModule('./stratus').default;
            return new stratusService(this);
        }, false);
    }
    connection(prop) {
