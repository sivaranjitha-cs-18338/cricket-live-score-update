...
export class DatastreamComponent implements OnInit, OnDestroy {
  ...
  // Base URL for your Catalyst function
  private readonly functionUrl = 'https://catalyst-region.project-domain.com/project-name/function-name';
  ...
  async loadMatches() {
    try {
      ...
      // const response = await this.http.get<CricketMatch[]>('your-cricket-api-endpoint').toPromise();
      
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      this.matches = this.mockMatches.map(match => ({
        ...match,
        // Simulate live score updates
        team1: {
          ...match.team1,
          score: match.live ? this.updateScore(match.team1.score) : match.team1.score
        },
        team2: {
          ...match.team2,
          score: match.live ? this.updateScore(match.team2.score) : match.team2.score
        }
      }));
      
      this.lastUpdated = new Date();
    } catch (error) {
      this.error = 'Failed to load cricket scores. Please try again.';
  ...
  private updateScore(currentScore: string): string {
    // Simulate random score updates for live matches
    if (Math.random() > 0.7) { // 30% chance of score update
      const parts = currentScore.split('/');
      if (parts.length === 2) {
        const runs = parseInt(parts[0]) + Math.floor(Math.random() * 6) + 1; // Add 1-6 runs
        return `${runs}/${parts[1]}`;
      }
    }
    return currentScore;
  ...
  startAutoRefresh() {
    ...
    this.refreshInterval = interval(30000).subscribe(() => {
      if (this.matches.some(match => match.live)) {
        this.loadMatches();
      }
    });
  }
